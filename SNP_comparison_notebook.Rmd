...
closest_match <- det_loci[ J( sim_loci ), roll = "nearest"]

CoordTable <- data.frame( cbind( sim=sim_loci, det=det_loci, match=closest_match )) %>% mutate( mismatch=det_loci-closest_match )
  names( CoordTable ) <- c("sim_coord", "call_coord", "best_match", "n_mismatch")
  cum_mismatch <- sum( CoordTable$n_mismatch ) / nrow( sim_GT )


```

The SNP loci are mis-located by a mean of `r cum_mismatch` base-pairs per locus:
